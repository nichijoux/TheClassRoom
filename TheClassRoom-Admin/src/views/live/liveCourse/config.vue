...
export default {
  ...
  methods: {
    ...
    // 根据id查询记录
    fetchDataById(id) {
      liveCourseAPI.getLiveCourseConfig(id).then((response) => {
        if (null !== response.data.id) {
          this.liveCourseConfigVo = response.data.liveCourseConfigVO;
        }
    ...
    fetchCourseList() {
      courseAPI.getAllCourseInfo().then((response) => {
        //debugger
    ...
    selectCourse() {
      ...
      this.multipleSelection.forEach((item) => {
        var obj = {
          ...
          tab: "1",
          url: "http://https://v1628504k8.oicp.vip/#/courseInfo/" + item.id,
          putaway: "1",
          pay: "1",
          qrcode: "",
        };
    ...
    saveOrUpdate() {
      liveCourseAPI
        .updateLiveConfig(this.liveCourseConfigVo)
        .then((response) => {
          this.$message({
            type: "success",
