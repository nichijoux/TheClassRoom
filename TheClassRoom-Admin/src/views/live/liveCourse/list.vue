...
export default {
  ...
  methods: {
    ...
    fetchData(index = 1) {
      ...
      liveCourseAPI.pageQueryLiveInfo(this.index, this.limit).then((response) => {
        this.list = response.data.lives;
    ...
    removeDataById(id) {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          // promise
          // 点击确定，远程调用ajax
          return liveCourseAPI.deleteLiveCourse(id);
        })
        .then((response) => {
    ...
    fetchDataById(id) {
      liveCourseAPI.getLiveCourse(id).then((response) => {
    ...
    // 新增
    saveData() {
      liveCourseAPI.addLiveCourse(this.liveCourse).then((response) => {
        if (response.code) {
          this.$message({
            type: "success",
    ...
    // 根据id更新记录
    updateData() {
      liveCourseAPI.updateLiveCourse(this.liveCourse).then((response) => {
        if (response.code) {
          this.$message({
            type: "success",
            message: response.message,
    ...
    showAccount(row) {
      ...
      liveCourseAPI.getLiveCourseAccount(row.id).then((response) => {
        this.liveCourseAccount = response.data.liveCourseAccount;
