...
export default {
  ...
  methods: {
    fetchData() {
      this.loading = true;
      courseAPI.userGetCourseInfo(this.courseId).then((response) => {
        this.courseVo = response.data.map.courseVo;
        this.description = response.data.map.description;
        this.isBuy = response.data.map.isBuy;
        this.chapterVoList = response.data.map.chapterVoList;
    ...
    wxRegister() {
      ...
      let url = window.location.href.replace("#", "tcr");
      shareAPI.getSignature(url).then((response) => {
        ...
        if (window.location.href.indexOf("?") != -1) {
          link =
            ...
            "&recommend=" +
            response.data.wxJsapiSignatureVO.userEedId;
        } else {
          link =
            window.location.href +
            "?recommend=" +
        ...
        wxShare.wxRegister(response.data.wxJsapiSignatureVO, option);
      });
    },
  },
};
