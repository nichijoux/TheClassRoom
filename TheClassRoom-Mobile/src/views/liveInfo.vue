...
export default {
  ...
  methods: {
    fetchData() {
      this.loading = true;
      liveAPI.userGetLiveCourseInfo(this.liveCourseId).then((response) => {
        this.liveCourse = response.data.map.liveCourse;
        this.description = response.data.map.description;
        this.teacher = response.data.map.teacher;
        this.liveStatus = response.data.map.liveStatus;
    ...
    play() {
      liveAPI.getAccessToken(this.liveCourseId).then((response) => {

        window.location = "./live.html?token=" + response.data.object.access_token;
        this.finished = true;
      });
    },
