package com.zh.tcr.live.mtcloud;

import com.zh.tcr.live.mtcloud.utils.MD5Util;
...
public class MTCloud {
    ...
    public String userAccessKey(String uid, String nickname, String role, String roomid, int expire) throws Exception {
        ...
        return this.base64UrlEncode(JSONObject.fromObject(params).toString());
    }
     ...
     * //     *  $zhuboKey 主播密码
     * //     *  $modetype 房间模式
     * //     *  $options  可选项，包括： barrage:弹幕开关 开启:1 关闭:0，departmentID：部门ID，user_top：最高在线用户数，streamMode：小班合流模式配置，1 多人模式，2 双人模式
     *
     * @return
     * @throws Exception
     */
     ...
     * <p>
     * roomid
     * params 包含字段：roomName，voiceFlow,authKey
     *
     * @return
     * @throws Exception
     */
     ...
     * account     主播账号
     * accountType 账号类型
     * filename    图片路径(支持jpeg、jpg，图片大小不超过1M)
     *
     * @return
     * @throws Exception
     */
     ...
     * <p>
     * course_id 课程ID
     * file      文件 {"file":"文件路径","name":"文件名"}，支持的课件格式为：ppt, pptx, doc, docx, pdf, jpg, jpeg, png, gif
     */
     ...
     * <p>
     * course_id 课程ID
     * filename  图片路径(支持图片格式:jpg、jpeg，图片大小不超过2M)
     *
     * @return
     * @throws Exception
     */
    ...
    public String doGet(String url) throws UnsupportedEncodingException {
        ...
        try {
            ...
            if (!Objects.equals(this.restUrl, this.restUrl2)) {
                return null;
            } else {
                e.printStackTrace();
            }
    ...
    private String mapToQueryString(HashMap<Object, Object> params) {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            try {
                if (i != array.length - 1) {
                    str.append(key).append("=").append(URLEncoder.encode((String) params.get(key), "UTF-8")).append("&");
                } else {
                    str.append(key).append("=").append(URLEncoder.encode((String) params.get(key), "UTF-8"));
                }
